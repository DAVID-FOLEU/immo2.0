...
function SearchLocation({ cars = [] }) {
  ...
  // Extractions dynamiques pour les menus déroulants basées sur ta BDD (en retirant les valeurs nulles)
  ...
  // Logique de filtrage mise à jour selon ton architecture SQL
  const filteredCars = useMemo(() => {
    const normalizedSearch = search.trim().toLowerCase()

    return cars.filter((car) => {
      // Extraction du prix de location de la BDD
      const actualPrice = Number(car.prix_location || 0)
  ...
  return (
    <>
      <section className="mb-4 p-4 bg-light rounded-3 shadow-sm">
        <h2 className="h5 mb-3 fw-bold text-dark">🔍 Recherche de voitures à louer</h2>
        <form className="row g-3">
          ...
          <div className="col-md-3">
            <label className="form-label small fw-semibold text-muted">Prix min / jour (F CFA)</label>
            <input
              type="number"
              className="form-control"
              placeholder="Minimum"
          ...
          <div className="col-md-3">
            <label className="form-label small fw-semibold text-muted">Prix max / jour (F CFA)</label>
            <input
              type="number"
              className="form-control"
              placeholder="Maximum"
      ...
      {filteredCars.filter(car => car.type_transaction === 'louer').length === 0 ? (
        <div className="alert alert-warning text-center">Aucun véhicule en location ne correspond à ces critères.</div>
      ) : (
        <div className="row">
          {filteredCars.map((car) => (
            // Filtrage strict basé sur la colonne type_transaction de ta base de données ('louer')
            car.type_transaction === 'louer' && <CardLocation key={car.id} car={car} />
          ))}
        </div>
      )}
    </>
